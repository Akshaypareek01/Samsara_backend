...
const teacherWaterIntakeSchema = new mongoose.Schema({
    teacherId: {
...
teacherWaterIntakeSchema.index({ teacherId: 1, 'dailySummary.date': 1 });

// Method to add water intake
teacherWaterIntakeSchema.methods.addIntake = async function(amount, note = '') {
...
teacherWaterIntakeSchema.methods.getTodaySummary = function() {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
...
teacherWaterIntakeSchema.methods.getDateHistory = function(date) {
...
export const TeacherWaterIntake = mongoose.model('TeacherWaterIntake', teacherWaterIntakeSchema); 